...
da.plot(kind='bar',stacked=True,title="The title of my graph")
```
...
```
by_species_sex = surveys_df.groupby(['plot','sex'])
pl_sx_count = by_species_sex['wgt'].sum()
```
...
```
by_species_sex = surveys_df.groupby(['plot','sex'])
sp_sx_count = by_species_sex['wgt'].sum()
sp_sx_count.unstack()
```
